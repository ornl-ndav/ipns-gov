 ...
 * multi(or uni)dimensional arrays of int,float,double,short,byte.  The GUI
 * allows for text entry of small lists.   Also, the value is passed by
 * reference. The specific data type for this ParameterGUI is determined
 * by the initial value.  All othe values are converted to that
 * real array type or if not possible, throws an exception.
 */
public class RealArrayPG extends ObjectPG_base {
	...
	Class baseClass;
	 ...
	 /**
	   ...
	   *
	   * @param name Prompt string for this RealArrayPG.
	   * @param val Value of this RealArrayPG. This determines the data type
	   *                    of this ParameterGUI henceforth
	   */
	public RealArrayPG(String name, Object val) throws IllegalArgumentException {
	...
	private static Object Test( Object val, Object thisval) throws 
	                                        IllegalArgumentException{

	   if( val == null) throw new IllegalArgumentException(" RealArray "+
				 "initial value must be a (multi dimensional) array");
	   Class CC = val.getClass();
	   if( !CC.isArray())throw new IllegalArgumentException(" RealArray "+
	              "initial value must be a (multi dimensional) array");
		if( thisval == null){
	...
	public Object getWidgetValue() throws IllegalArgumentException {
		return Conversions.get_RealArray( Conversions.StringToVec(entryField.getText()), obj_value.getClass());
	}
	...
	public Object getObjectValue(Object obj) throws IllegalArgumentException {
		if( obj_value == null)// hopefully this happens only at construction time
				return obj;
		if( obj.getClass().equals( obj_value.getClass()))
			return obj;
		return Conversions.get_RealArray(obj, obj_value.getClass());//Should not do this
	}
	...
	public JPanel getWidget() {
	   ...
	   /**
		 * Set internal references to the lower level GUI entry widget to null
		 * so that it can be garbage collected. Subsequent calls to getWidget()
		 * will create a new widget.
		 */
