...
/**
 * This class implements a sine function with fixed period
 */

public class SineFixedPeriod extends    OneVarParameterizedFunction
                              implements Serializable
{
  /**
   *  Construct a new sin function object with the specified
   *  characteristics 
   *
   *  @param  amplitude
   *  @param  phase
   ...
   */
   ...
   public SineFixedPeriod( double amplitude, double phase, double background )
   {
     super( "sinep", new double[3], new String[3] );
     parameter_names[0] = "Amplitude";
     parameter_names[1] = "Phase";
     parameter_names[2] = "Background";
  ...
  {    
    if ( domain.contains( (float)x ) )
      return parameters[0] * Math.sin(2*Math.PI * x / period + parameters[1]) + parameters[2];
    else
      return 0; 
  }
  ...
  public double[] getValues( double x[] )
  {
    ...
    { 
      ...
      {
        vals[i] = parameters[0] * Math.sin(2*Math.PI * x[i] / period + parameters[1]) + parameters[2];
      }
      else
        vals[i] = 0;
    }
  ...
  public float getValue( float x )
  {
    ...
    if ( domain.contains( x ) )
      return (float)( parameters[0] * Math.sin(2*Math.PI * x / period + parameters[1]) + parameters[2] );
    else
      return 0;
  }
  ...
  public float[] getValues( float x[] )
  {
    ...
    for ( int i = 0; i < x.length; i++ )
    {
      if ( x[i] >= x_min && x[i] <= x_max )
      {
        vals[i] = (float)(parameters[0] * Math.sin(2*Math.PI * x[i] / period + parameters[1]) + parameters[2]);
      }
      else
        vals[i] = 0.0f;
    }
  ...
  */
    public static void main(String[] args)
    {
      ...
      SineFixedPeriod wave = new SineFixedPeriod( 1, 2, 3);
