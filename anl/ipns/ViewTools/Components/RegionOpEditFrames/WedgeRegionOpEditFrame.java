...
/**
 * This class creates a Wedge Region Editor based on three defining 
 * points in the world coordinate system
 * @author Josh Oakgrove
 *
 */
public class WedgeRegionOpEditFrame extends JFrame implements RegionOpEditFrame
{
private static int VALUE_JUMP = 5;
   ...
   */
  public WedgeRegionOpEditFrame(floatPoint2D pt1, floatPoint2D pt2,
      floatPoint2D pt3,RegionOp.Operation op,int index)
  {
    super("Wedge Editor");
    this.setBounds(700,390, 350, 250);
...
//      set approprate values for draw
      ...
      else if(message.equals("Complement"))
      {
        ...
        Float newX = rotateX-2*(rotateX-focusX);
        Float newY = rotateY-2*(rotateY-focusY);
        p3x.setText(""+newX);
        p3y.setText(""+newY);
        this_editor.firePropertyChange(DRAW_CURSOR,1,2);
      }
...
//        set approprate values for draw
  ...
  {
    WedgeRegionOpEditFrame test = new WedgeRegionOpEditFrame();
